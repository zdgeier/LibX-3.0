...
export default class Catalog {
    ...
    // }
    makeKeywordSearch(/**String*/keyword) {
        return this.makeSearch("Y", keyword);
    }
    /**
    ...
    search = function (fields) {
        ...
        if (fields.length == 1) {//single search field
            url = this.makeSearch(fields[0].searchType, fields[0].searchTerms);
        } else {// user requested multiple search fields, do advanced search
            url = this.makeAdvancedSearch(fields);
        }
        if (url == null) {
            console.log("Could not construct search");
        }
        return url;
    }
    ...
    combineSameTypedFields = function (fields) {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            var searchType = fields[i].searchType;
